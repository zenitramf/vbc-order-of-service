...
export const TemplateEditorPage = ({
  ...
  const [templateJson, setTemplateJson] = React.useState<OrderServiceTemplateJson>(
    ...
    <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">
            Build a reusable service plan. Saving updates the matching service type for future orders.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Name the template and service type users will choose when creating orders.</CardDescription>
        </CardHeader>
        <CardContent>
          <FieldGroup>
            <Field>
              <FieldLabel htmlFor="template-name">Template and service type name</FieldLabel>
              <Input
                id="template-name"
                onChange={(event) => {
                  setName(event.target.value);
                  setTemplateJson((current) => ({ ...current, name: event.target.value }));
                }}
                value={name}
              />
              <FieldDescription>
                Examples: Sunday Service, Thursday Service, Revival, Missions Conference.
              </FieldDescription>
            </Field>
          </FieldGroup>
        </CardContent>
