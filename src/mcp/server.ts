...
export const createVbcMcpServer = (caller: McpCaller): McpServer => {
  ...
  server.registerTool(
    ...
    {
      description:
        "List hymns with music key, source, last played, and recent play count. All filter fields are optional and can be combined.",
      inputSchema: {
        hymnNumber: z
          .string()
          .optional()
          .describe("Filter by hymn number (exact match)"),
        lastPlayedFrom: z
          .string()
          .optional()
          .describe(
            "Include hymns last played on or after this date (YYYY-MM-DD or ISO datetime)"
          ),
        lastPlayedTo: z
          .string()
          .optional()
          .describe(
            "Include hymns last played on or before this date (YYYY-MM-DD or ISO datetime)"
          ),
        maxTimesPlayedLastSixMonths: z
          .number()
          .int()
          .min(0)
          .optional()
          .describe("Maximum play count in the last 6 months (inclusive)"),
        minTimesPlayedLastSixMonths: z
          .number()
          .int()
          .min(0)
          .optional()
          .describe("Minimum play count in the last 6 months (inclusive)"),
        musicKey: z
          .string()
          .optional()
          .describe("Filter by music key (exact match)"),
        neverPlayed: z
          .boolean()
          .optional()
          .describe("When true, only return hymns that have never been played"),
        search: z
          .string()
          .optional()
          .describe("Case-insensitive substring match on hymn number or name"),
        sourceName: z
          .string()
          .optional()
          .describe("Filter by hymn source name (exact match)"),
        timesPlayedLastSixMonths: z
          .number()
          .int()
          .min(0)
          .optional()
          .describe("Exact play count in the last 6 months"),
      },
    },
    (filters) =>
      withToolErrors(async () => {
        ...
        return jsonToolResult(filterHymns(hymns, filters));
      })
  );
